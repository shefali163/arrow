  ...
  void SetUp() override {
    ...
    }
    {
      if (isHeirarchialNamespaceEnabled()) {
        ...
        auto fc = std::make_shared<Azure::Storage::Files::DataLake::DataLakeFileClient>(options_.account_blob_url + "container/somefile", options_.storage_credentials_provider);
        std::string s = "some data";
        fc->UploadFrom(const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(&s[0])), s.size());
      }
    }
  }
...
TEST_F(TestAzureFileSystem, CopyFileBlobStorage) {
  ...
  ASSERT_RAISES(IOError, fs_->CopyFile("container23/somedir/subdir", "container/newfile"));
  ...
  ASSERT_RAISES(IOError, fs_->CopyFile("container/somedir/subdir/subfile", "container3435"));
  ...
  ASSERT_RAISES(IOError, fs_->CopyFile("container/somedir/subdir/subfile", "container/sjdj"));
  ASSERT_RAISES(IOError, fs_->CopyFile("container/somedir/subdir/subfile", "container/somedir/subdir"));
  ASSERT_RAISES(IOError, fs_->CopyFile("container/somedir/subdir/subfile", "container/ahsj/ggws"));
  ASSERT_RAISES(IOError, fs_->CopyFile("container/somedir/subdir/subfile", "container27/hshj"));
  ASSERT_RAISES(IOError, fs_->CopyFile("container/somedir/subdir/subfile", "container27/hshj"));
}
